...
import {
  ...
  replaceCamelCaseWithUnderscores,
  toCamelCase,
  toCapitalCase,
  Separator,
...
import { QUESTIONS } from "../resources/en.resource.js";
import {
  ColumnDetails,
  DEFAULT_PROPERTIES,
} from "../resources/default_properties.constant.js";
enum Driver {
  MySql = "MySql",
  Postgres = "Postgres",
  MariaDB = "MariaDB",
...
export const askForEntityName = async (): Promise<string> => {
  ...
  const randomEntityName = toCapitalCase(
    ENTITIES_NAME_SAMPLES[randomIndex],
...
export const askForTableName = async (entityName?: string): Promise<string> => {
  if (entityName) {
    return askQuestion("TABLE_NAME", {
      type: "input",
      name: "table_name",
      default: replaceCamelCaseWithUnderscores(
        replaceSpacesWithUnderscores(pluralize(entityName))
      ),
    });
  } else {
...
export async function askColumnDetails(): Promise<ColumnDetails> {
  ...
  if (DEFAULT_PROPERTIES[columnName]) {
    ...
    console.log(
      "Propriétés prédéfinies pour la colonne:",
      DEFAULT_PROPERTIES[columnName]
    );
...
export async function askColumnToModify(
